...
void ZString::DeserializeFromJson(ZString& string, const char* chars)
{
	const size_t length = strlen(chars);
	char* chars2 = new char[length + 1];

	strncpy_s(chars2, length + 1, chars, length + 1);

	string.m_length = length | 0x80000000;
	string.m_chars = chars2;
}
