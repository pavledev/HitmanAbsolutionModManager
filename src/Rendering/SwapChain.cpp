...
bool SwapChain::Create(ID3D11Device* d3dDevice, HWND hWnd)
{
	...
	if (FAILED(result))
	{
		Logger::Error("Failed to query IDXGIDevice interface!");
	...
	if (FAILED(result))
	{
		Logger::Error("Failed to get factory!");
...
bool SwapChain::CreateRenderTargetView(ID3D11Device* d3dDevice)
{
...
bool SwapChain::Resize(ID3D11Device* d3dDevice, unsigned int width, unsigned int height, const bool force)
{
	...
	if (true) // DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH
	{
		...
		if (FAILED(result))
		{
			Logger::Error("Failed to resize swapchain target!");
	...
	if (FAILED(result))
	{
		Logger::Error("Failed to resize swapchain buffers!");
...
void SwapChain::Present()
{
	if (!swapChain)
	{
		Logger::Error("Can't present, the swapchain has not been initialised!");
	...
	if (FAILED(result))
	{
		Logger::Error("Failed to present!");
	}
}
...
SwapChain::PresentMode SwapChain::GetSupportedPresentMode(const PresentMode presentMode)
{
	...
	if (presentMode == PresentMode::Immediate)
	{
		...
		{
			Logger::Warn("PresentMode::Immediate is not supported, falling back to PresentMode::Fifo!");
