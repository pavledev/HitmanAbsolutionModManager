    ...
    static TArray* DeserializeFromJson(const rapidjson::Value& jsonArray)
    {
        ...
        for (rapidjson::Value::ConstValueIterator it = jsonArray.Begin(); it != jsonArray.End(); ++it)
        {
            if constexpr (std::is_class_v<T>)
            {
                ...
                {
                    T::DeserializeFromJson((*array)[index], it->GetString());
                }
                else
                {
                    if (it->IsObject())
                    {
                        ...
                        T::DeserializeFromJson((*array)[index], object);
                    }
                    else
                    {
                        const rapidjson::Value& array2 = it->GetArray();

                        T::DeserializeFromJson((*array)[index], array2);
                    }
                }
            }
            else
            {
                ...
                {
                    (*array)[index] = it->GetInt();
                }
                else if constexpr (std::is_enum_v<T>)
                {
                    (*array)[index] = static_cast<T>(it->GetUint());
                }
                else if constexpr (std::is_same_v<T, unsigned char> || std::is_same_v<T, unsigned short> || std::is_same_v<T, unsigned int>)
                {
                    (*array)[index] = it->GetUint();
                }
                else if constexpr (std::is_same_v<T, long long>)
                {
                    (*array)[index] = it->GetInt64();
                }
                else if constexpr (std::is_same_v<T, unsigned long long>)
                {
                    (*array)[index] = it->GetUint64();
                }
                else if constexpr (std::is_same_v<T, float>)
                {
                    (*array)[index] = it->GetFloat();
                }
                else if constexpr (std::is_same_v<T, double>)
                {
                    (*array)[index] = it->GetDouble();
                }
                else if constexpr (std::is_same_v<T, bool>)
                {
                    (*array)[index] = it->GetBool();
                }
            }
