...
public:
    ...
    bool Create(ID3D11Device* d3dDevice, HWND hWnd);
    bool CreateRenderTargetView(ID3D11Device* d3dDevice);
    bool Resize(ID3D11Device* d3dDevice, unsigned int width, unsigned int height, const bool force = false);
    void Present();
